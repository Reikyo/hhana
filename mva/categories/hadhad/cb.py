...
JET_SUBLEAD = Cut('jet_1_pt > 30')
MJJ = Cut('dijet_vis_mass > 300') # ??? Is this the same as MASS_JETS above ??? Replace ??? Units ???
DETA_JETS_CBVBF = Cut('dijet_deta > 3.0') # ??? Name okay, change to DETA_JETS now this has been commented above ???
ETA_PRODUCT1 = Cut('(jet_0_eta < 0) && (jet_1_eta > 0)') # ??? >= or <= ???
ETA_PRODUCT2 = Cut('(jet_0_eta > 0) && (jet_1_eta < 0)') # ??? >= or <= ???
ETA_PRODUCT = ETA_PRODUCT1 | ETA_PRODUCT2
TAU0_TOPO1 = Cut('(ditau_tau0_eta > jet_0_eta) && (ditau_tau0_eta < jet_1_eta)') # ??? >= or <= ???
TAU0_TOPO2 = Cut('(ditau_tau0_eta < jet_0_eta) && (ditau_tau0_eta > jet_1_eta)') # ??? >= or <= ???
TAU0_TOPO = TAU0_TOPO1 | TAU0_TOPO2
TAU1_TOPO1 = Cut('(ditau_tau1_eta > jet_0_eta) && (ditau_tau1_eta < jet_1_eta)') # ??? >= or <= ???
TAU1_TOPO2 = Cut('(ditau_tau1_eta < jet_0_eta) && (ditau_tau1_eta > jet_1_eta)') # ??? >= or <= ???
TAU1_TOPO = TAU1_TOPO1 | TAU1_TOPO2
...
class Category_Cuts_VBF_HighDR(Category_Preselection):
    ...
    linestyle = 'longdash'
    cuts = (
          Category_Cuts_VBF_HighDR_Tight.cuts
        | Category_Cuts_VBF_HighDR_Loose.cuts)
...
class Category_Cuts_VBF(Category_Preselection):
    ...
    latex = '\\textbf{VBF}'
    cuts  = (
          Category_Cuts_VBF_LowDR.cuts
        | Category_Cuts_VBF_HighDR.cuts)
    norm_category = Category_Preselection
...
class Category_Cuts_Boosted(Category_Preselection):
    ...
    latex = '\\textbf{Boosted}'
    cuts = (
          Category_Cuts_Boosted_Tight.cuts
        | Category_Cuts_Boosted_Loose.cuts)
    norm_category = Category_Preselection
...
class Category_Cuts(Category_Preselection):
    name = 'cuts'
    label = '#tau_{had}#tau_{had} CB All Cuts'
    latex = '\\textbf{All Cuts}'
    common_cuts = Category_Preselection.common_cuts
    cuts = (
          Category_Cuts_VBF.cuts
        | Category_Cuts_Boosted.cuts)
    norm_category = Category_Preselection
